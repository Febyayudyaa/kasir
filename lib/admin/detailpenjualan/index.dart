...
class _IndexDetailJualState extends State<IndexDetailJual> {
  ...
  Future<void> fetchDetailPenjualan() async {
    ...
    try {
      ...
      setState(() => detailpenjualan = List<Map<String, dynamic>>.from(response));
    } catch (e) {
  ...
  Future<void> transaksi(int pelangganID, int subtotal) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(response.isNotEmpty ? 'Pesanan berhasil disimpan!' : 'Gagal menyimpan pesanan')),
      );
    ...
    return Scaffold(
      ...
      body: isLoading
          ...
          : detailpenjualan.isEmpty
              ? const Center(
                  child: Text('Detail penjualan tidak ada', style: TextStyle(fontSize: 18)),
                )
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final int subtotal = (detail['Subtotal'] is int)
                        ? detail['Subtotal']
                        : int.tryParse(detail['Subtotal']?.toString() ?? '0') ?? 0;
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            ElevatedButton(
                              onPressed: isOrdering ? null : () async => await transaksi(pelangganID, subtotal),
                              ...
                              child: isOrdering
                                  ...
                                  : const Text('Pesan', style: TextStyle(fontSize: 16, color: Colors.white)),
                            ),
                          ],
                        ),
                      ),
